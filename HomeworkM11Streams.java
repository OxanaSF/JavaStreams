...
public class HomeworkM11Streams {
	...
	public static void main(String[] args) throws Exception {
		...
		// Q13: Find the average spending in the state of Illinois.
		System.out.print("\nQ13: How much money on average have customers in state Illinois spent: ");
		double averageSpentInIllinois = q13();
		System.out.println(averageSpentInIllinois);
	...
	public static long q1() {
		...
		return customerList.stream()
				.filter(c -> c.getState().equalsIgnoreCase("CA"))
				.count();
	}
	public static List<Customer> q2 () {
		// Q2: Create a list of all priority customers in MA.
		return customerList.stream()
				.filter(c -> c.getState().equalsIgnoreCase("MA") && c.isPriority())
	...
	public static Map<String, Customer> q5() {
		// Q5: Create a map of all WY priority customers (key=id, value=customer)
		return customerList.stream()
				.filter((c -> c.getState().equalsIgnoreCase("WY") && c.isPriority()))
				.collect(Collectors.toMap(Customer::getId, Function.identity()));
	}
	public static double q6() {
		// Q6: What is the greatest amount of money spent by a NY priority customer?
		return customerList.stream()
				.filter(c -> c.getState().equalsIgnoreCase("NY") && c.isPriority())
				.mapToDouble(Customer::getAmountSpent)
				.max()
				.orElse(0);
	}
	public static String q7() {
		...
		// Create a comma-separated String of all customer IDs for customers that spent > 9000:
		return customerList.stream()
				.filter(c -> c.getAmountSpent() > 9000)
				.map(Customer::getId)
	...
	public static void q8() {
		...
		customerList.stream()
				...
				.findFirst()
				.ifPresent(c -> System.out.println("Amount: " + c.getAmountSpent()));
	}
	public static int q9(String[] numWords) {
		// Q9: Find the sum of the numbers represented in an String array.
		return Arrays.stream(numWords)
	...
	public static void q12(List<String> scrabbleWords) {
		...
		scrabbleWords.stream()
				.sorted((word1, word2) -> score(word2) - score(word1))
				.limit(9)
				.forEach(word -> System.out.println(word + " : " + score(word)));
	}

	public static double q13() {
		// Q13: How much money on average have customers in state Illinois spent?
		return customerList.stream()
				.filter(c -> c.getState().equalsIgnoreCase("IL"))
				.mapToDouble(Customer::getAmountSpent)
				.average()
				.orElse(0);
	}
